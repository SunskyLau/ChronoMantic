...
def add_predictions_to_results(query_results: dict, pattern_type: str, df: pd.DataFrame) -> None:
    ...
    # 计算过滤阈值
    min_duration = total_time_span / 250  # 最小时间长度阈值
...
def find_best_matches(
    predictions: List[Tuple[float, float]], ground_truths: List[Tuple[float, float]], iou_threshold: float = 0.75
    ...
    for target_name, target_results in results.items():
        ...
        for pattern_type, pattern_data in target_results.items():
            ...
            if not ground_truths:
                ...
                # 找到最佳匹配
                matches = find_best_matches(predictions, ground_truths)
...
def execute_pattern_queries(approximation_segments_containers: dict, df: pd.DataFrame) -> None:
    ...
    # 1. Peak模式查询
    peak_query = QuerySpec(
        targets=targets, trends=[Trend(category=TrendCategory.UP), Trend(category=TrendCategory.DOWN)], single_relations=[], trend_groups=[], group_relations=[]
    )
    peak_results = query(peak_query, approximation_segments_containers, df)
    add_predictions_to_results(peak_results, "peak", df)

    # 2. Valley模式查询
    valley_query = QuerySpec(
        targets=targets, trends=[Trend(category=TrendCategory.DOWN), Trend(category=TrendCategory.UP)], single_relations=[], trend_groups=[], group_relations=[]
    )
    valley_results = query(valley_query, approximation_segments_containers, df)
    add_predictions_to_results(valley_results, "valley", df)
    ...
    # 8. Inverted-head-and-shoulders模式查询
...
def main() -> None:
    ...
    ground_truth_file = os.path.join(script_dir, "final_portfolio_data.csv_annotations.json")
    ...
    for target_name, target_results in results.items():
        print(f"\n目标: {target_name}")
        for pattern_type, pattern_data in target_results.items():
            predictions_count = len(pattern_data["predictions"])
            ground_truths_count = len(pattern_data["ground_truths"])
            print(f"  {pattern_type}: 预测 {predictions_count} 个, 真实 {ground_truths_count} 个")
