...
def test_parse_nl_query():
    ...
    # client = myAIClient(model=Qwen.MODELS.QWEN_TURBO, platform=Platforms.QWEN)
    # client = myAIClient(model=GroqPlatform.MODELS.LLAMA3_3, platform=Platforms.GROQ)
    # client = myAIClient(model=Qwen.MODELS.QWEN_2_5_32B_INSTRUCT, platform=Platforms.QWEN)
    # client = myAIClient(model=Ollama.MODELS.GEMMA3_12B, platform=Platforms.OLLAMA)
    ...
    response = client.send_prompt(parse_nl_user_prompt, client.platform != Platforms.QWEN)
