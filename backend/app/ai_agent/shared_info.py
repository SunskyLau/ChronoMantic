...
parse_logic_info = """
1. 自然语言中如果出现模糊的范围表达，解析成ScopeConditionWithSource的时候需要让min和max构成一个满足模糊表达的范围，min和max不应该相等。例如，"about 2 weeks"需要解析成min对应12days，max对应16days的ScopeConditionWithSource，也就是允许一定的范围来满足模糊化的表达。
2. 自然语言中对于趋势和形状的描述，需要解析成TrendWithSource，其中category需要解析成趋势的类别，text_source需要解析成趋势的描述来源。比如，"rise"需要解析成"up"，"fall"需要解析成"down"，"constant"需要解析成"flat"。另外，形状的描述通常是趋势的组合，比如"two-tops"需要解析成["up","down","up","down"]的组合，"head-and-shoulders"需要解析成["up","down","up","down","up","down"]的组合。
...
6. 你应该使用python脚本来获取精确的字符index和进行精确的计算，例如你应该使用python脚本获取"rose sharply"中的"sharply"的精确index，你也应该使用python脚本获取"about 2 weeks"中的12days和16days的精确秒数
"""