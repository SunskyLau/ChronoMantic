...
5. TextSource的text只能是来源original_text的子文本，TextSource是用于在系统中高亮文本的，因此TextSource不能出现重叠情况，否则系统无法进行正确的高亮。例如，"two consecutive rises", 如果"rises"被解析为一个TextSource，那么"two consecutive rises"就不应该被解析为TextSource，因为"two consecutive rises"已经包含了"rises"，不允许出现重叠的TextSource。
...
8. 注意trend_group的text_source是再最外层的，而不是在time_span_condition中的。
"""
